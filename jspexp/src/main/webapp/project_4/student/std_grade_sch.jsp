	...
	<div>
		<div>
			<div class="flex-box1">
				...
				<div id="top-box2">
					<span id="tob-box2-span">ID ${mem.id} 학생</span> <span
						id="tob-box2-span"><input type="button" id="bt2"
						value="로그아웃" onclick="logout()" /></span>
				</div>
			</div>
			<div class="flex-box2">
					...
					<div class="main-box-flex">
						<div id="main-box2">
							<div>
								...
								<jsp:useBean id="sch" class="jspexp.project_4.vo.Grade" />
								<jsp:setProperty property="*" name="sch" />
								<c:if test="${empty param.lecYear}">${sch.setLecYear("")}</c:if>
								<c:if test="${empty param.semester}">${sch.setSemester("")}</c:if>
								<form method="post">
									<h5
										style="margin-left: 40px; display: inline-block; color: #A0A0A0;">년도선택</h5>
									<select class="sel01" name="lecYear">
									...
									<h5
										style="margin-left: 15px; display: inline-block; color: #A0A0A0;">학기선택</h5>
									<select class="sel01" name="semester">
										<option value="1">1</option>
										<option value="2">2</option>
									</select> <input class="btn" type="submit" value="조회" />
								</form>
