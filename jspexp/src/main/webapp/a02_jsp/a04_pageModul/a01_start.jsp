...
0. 화면을 구성할 때는 여러가지 설정이 필요한 경우도 있고, 공통으로 처리하는 페이지가 
	필요할 때도 많다. 공통 처리 내용에 대하여 따로 jsp을 선언하고 활용한다면
	...
	1) 페이지 모듈화는 크게 
		...
		- 서버 프로그램을 모듈화
			...
			하나의 jsp로 최종적으로<%@ include file=""/>
				main.jsp/sub.jsp가 결국은 하나의 객체로 만들어져서 모듈화된 내용
				하나의 파일로 변수를 상호 공유한다.
				주로 실무에서 활용 되는 코드
				상단에 session 설정, js코드와 함께 처리하는 내용
				자바 코드 + 공통화면 + js/css 등 모두 선언하여
				사용할 수 있다.
	2) BOM, DOM의 계층 구조
		- 계층 구조
			...
			상위 ==> 하위 접근
				BOM.DOM.속성
				ex) frame01.document.querySelector("h1").innerText
...
1. jsp는 외부페이지를 모듈화하여 포함되어 처리할 수 있다.
	jsp ==> java 라는 개념에 의해, 외부의 다른 컴파일된 파일을 포함시켜서 처리 하는 방법과
	포함되어서 하나의 컴파일로 파일을 처리하는 방법으로 두가지로 나누어서 처리한다.
	1) 독림된 java파일을 호출하여 처리하는 경우
		<jsp:include 액션 태그
...
--%>
	...
	// request 범위로 데이터를 가지고 있을려면, 이렇게 매번 forward 처리해야
	// 가능하지만,
...
2. 기본형식
	...
	ps) 버퍼개념 - 화면을 구현하는데 어느 저이도의 데이터가 있는 패킷(데이터단위)가 있어야 표현이 되는데,
		네트워크 속도가 좋지 않을 때, 현재 화면을 조금씩 로딩하는 것보다 모든 내용을 다 받아서 로딩하는
		것이 효율적일 때, 버퍼라는 메모리를 써서 한꺼번에 화면을 구성하는 개념을 말한다.
		A화면 ==> B화면
		ex) 비유 - 1ton의 물이 이용해서 우물을 완성할 때 조금씩 양동이를 가지고 나르다가 다 차면
			한번에 완료를 하는 개념이다.
3. <jsp:param>액션 태그
	1) 신규 파라미터를 추가하는데 사용
...
--%>
	...
	<%--
	ex) a03_includeExp.jsp 에 include액션태그로 a04_subPage.jsp를 
		만들고, 출력하세 하세요.
		전달데이터 - 회원명 회원권한
