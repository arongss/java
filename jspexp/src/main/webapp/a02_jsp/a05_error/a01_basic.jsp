...
2. 위와 같은 기본적인 에러 처리를 바탕으로 jsp는 화면단이 추가 되기에 에러가 발생시,
	화면을 어떻게 처리해주어야 하는 부분이 추가적으로 처리되어야 한다.
3. 그래서, jsp는 에러가 발생했을 때, 에러가 나는 복잡한 화면대신에 처리하는 화면을
	출력하는데, 크게 두가지 방식으로 처리한다.
	1) 에러페이지를 만들고, 각 ㅔ이지마다 페이지 지시자로 에러가 발생했을 때,
		출력할 jsp 페이지를 지정해서 처리하는 것과
	2) web.xml에 에러페이지를 지정하여, 서버단위로 방생하는 모든 에러에 대하여
		지정한 페이지를 로딩하게 한다.
	3) 우선순위는 가장 세부적인 설정이 처리가 된다.
		즉, web.xml는 전체 에러
		각 페이지마다 페이지 지시자로 에러 처리(우선순위)
4. 서버 단위 에러 처리 방식
	1) web.xml를 통해 이 ㅓ버를 통해 처리되는 모든 예외를 대신한 페이지를 처리할 수 있는데,
...
1. 에러가 발생했을 때, 나타낼 페이지 구현
	1) 지시자 선언
		...
		이렇게 속성값을 지정하면, 
		예외의 기본 객체의 참조변수 exception을 처리할 수 있다.
		ex) <%=exception.getMessage()%>
		ex) <%=exception.printStackTrace()%>
		이 부분은 자바관련 코드 예외가 발생시만 처리할 수 있기 때문에
		500이외에 에러에 대해서는 exception클래스가 생성되지 않을
		수 있기에 전체 에러를 한꺼번에 처리할려면,
...
1. 서버의 설정페이지 web.xml에서 응답코드(http)별로
	...
	ps) 응답코드
		...
		500 : 서버 내부에서 오류가 발생 - 프로그램적 요류(java를 클래스예외는 이 경우에 해당함)
...
4. 예외 처리의 우선순위
	0) 예외 처리는 중첩적으로 처리될 때, 우선순위가 아래의 조건에 따라 처리된다.
		- web.xml에서 선언한 예외는 전체 서버 예외 이기에,
			개별페이지에서 예외처리를 하면 해당 개별페이지가 처리된다.
		- web.xml에서 500전체 서버에러에 대한 내용은
			개별적인 예외 처리보다 우선순위가 뒤로 된다.
			# 우선순위
			500 < Exception < 개별적인예외클래스
	
ex) z01_errorPage.jsp로 경로로 설정하고, web.xml에
	500에러 처리를 한 후, 
	테스트 페이지(a04_server_error.jsp-nullpointer강제 발생)에서 처리한 결과를
	확인하세요..
