...
2. 집합이나 콜렉션 데이터 사용
	<c:forEach var="변수" items="배열/객체형배열" varStatus="sts"
		...
		${sts.last}  마지막 데이터일 떼, boolean true
--%>
...
<%-- ex1) 특정횟수 반복 1~9 번호와 함께 하는 테이블 생성 --%>
<table>
...
<%-- ex2) 과일가게 판매물품 배열로 9개 선언하고 3X3테이블로 출력처리 (2번째 forEach로) --%>
<%
	request.setAttribute("fruits",new String[]{"사과","바나나","딸기","복숭아","오렌지","키위","망고","수박","앵두"});
%>
<table>
	<c:forEach var="fruit" items="${fruits}" varStatus="sts">
