...
%>
	...
	<h2>다중 선택</h2>
	<form>
		<input type="checkbox" name="loc" value="인도"/>인도
		<input type="checkbox" name="loc" value="남태평양"/>남태평양
		<input type="checkbox" name="loc" value="제주도"/>제주도
		<input type="checkbox" name="loc" value="유럽"/>유럽<br>
	...
	<h2>단일 선택</h2>
	<form>
		<input type="radio" name="loc" value="인도"/>인도
		<input type="radio" name="loc" value="남태평양"/>남태평양
		<input type="radio" name="loc" value="제주도"/>제주도
		<input type="radio" name="loc" value="유럽"/>유럽<br>
		<input type="submit" value="확인">
	</form>
	<% // radio는 name이 동일한 내용에서 단일 선택만 가능
	String loc = request.getParameter("loc");
	%>
	<h3>선택한 여행지:<%=loc!=null?loc:""%></h3>
<%
...
%>
	...
	<table>
		...
		<%} %>
		<tr><td colspan="4"><input type="submit" value="결재하기"/></td>
	</table>
	</form>
	<%--
	ch : 요청값은 checkbox이기에 체크한 index만 전송
		{0, 3}
	pnames : [사과,바나나,딸기,오렌지] 	pname[0] pnames[3]
	prices : [3000,4000,12000,1200]	price[0] prices[3]
	cnts : [2,5,3,1]  				cnt[0] cnts[3]
	 --%>
	...
	if(ch!=null){
		...
		for(String cidx:ch){
			...
			out.print(cidx+","+pnames[idx]+":"+prices[idx]
					+":"+cnts[idx]+"<br>");
		}
	}
	%>
