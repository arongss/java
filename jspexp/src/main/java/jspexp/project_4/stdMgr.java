...
import jspexp.project_4.vo.Lecture01;
import jspexp.project_4.vo.Lecture02;
import jspexp.project_4.vo.Major;
import jspexp.project_4.vo.Notice;
import jspexp.project_4.vo.Tuition;
...
public class stdMgr {
	...
	public List<Notice> checkTitle(Notice sch) {
		List<Notice> nlist = new ArrayList<Notice>();
		String sql = "SELECT noticeNo,title,NOTICEDATE,VIEWS FROM notice WHERE title LIKE '%'||?||'%'";
		try {
			con = DB.con();
			...
			while (rs.next()) {
				nlist.add(new Notice(rs.getInt("noticeNo"), rs.getString("title"), rs.getString("noticedate"),
						rs.getInt("views")));
			}
	...
	public List<Notice> checkContents(Notice sch) {
		List<Notice> nlist = new ArrayList<Notice>();
		String sql = "SELECT noticeNo,title,NOTICEDATE,VIEWS FROM notice WHERE contents LIKE '%'||?||'%'";
		try {
			con = DB.con();
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, sch.getContents());
			rs = pstmt.executeQuery();
			while (rs.next()) {
				nlist.add(new Notice(rs.getInt("noticeNo"), rs.getString("title"), rs.getString("noticedate"),
						rs.getInt("views")));
	...
	public List<Lecture01> checkLecName(Lecture01 sch) {
		...
		String sql = "SELECT lecNum,majorName,class_l,sort,lecName,lecLoc,times,grade FROM "
		...
		try {
			...
			rs = pstmt.executeQuery();
			while (rs.next()) {
				llist.add(new Lecture01(rs.getInt("lecNum"), rs.getString("majorName"), rs.getInt("class_l"),
						rs.getString("sort"), rs.getString("lecName"), rs.getString("lecLoc"), rs.getString("times"),
						rs.getInt("grade")));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public List<Lecture01> checkMajor(Lecture01 sch) {
		List<Lecture01> llist = new ArrayList<Lecture01>();
		String sql = "SELECT lecNum,majorName,class_l,sort,lecName,lecLoc,times,grade from\r\n"
				+ "(SELECT lecNum,majorName,class_l,sort,lecName,lecLoc,times,grade"
	...
	public List<Lecture01> checkClass(Lecture01 sch) {
		...
		String sql = "SELECT lecNum,majorName,class_l,sort,lecName,lecLoc,times,grade from\r\n"
				+ "(SELECT lecNum,majorName,class_l,sort,lecName,lecLoc,times,grade "
				+ "FROM lecture l,major m WHERE l.majorNo=m.majorNo)\r\n"
				+ "WHERE class_l=? and majorName LIKE '%'||?||'%'";
		try {
			con = DB.con();
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, sch.getClass_l());
	...
	public List<Lecture01> getStdLecture(String id) {
		List<Lecture01> llist = new ArrayList<Lecture01>();
		String sql = "SELECT l.lecNum,majorName,class_l,sort,lecName,lecLoc,times,grade FROM\r\n"
				+ "lecture l,major m,STDLECTURE s  " + "WHERE l.majorNo=m.majorNo AND s.LECNUM = l.LECNUM AND s.id=?";
		try {
			con = DB.con();
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, id);
...
//			System.out.println(l.getMajorName()+","+l.getClass_l());
//		}
