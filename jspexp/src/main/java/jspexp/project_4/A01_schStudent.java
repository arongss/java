...
import jspexp.project_4.vo.Lecture_p;
import jspexp.project_4.vo.Student_p;
...
public class A01_schStudent {
	...
	public List<Lecture_p> getLecSch(Lecture_p sch) {
		List<Lecture_p> list = new ArrayList<Lecture_p>();
		...
		try {
			...
			while (rs.next()) {
				Lecture_p e = new Lecture_p(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4),
						rs.getString(5));
				list.add(e);
			}
	...
	public List<Student_p> schStu(Student_p sch) {
		List<Student_p> slist = new ArrayList<Student_p>();
		...
		try {
			...
			rs = pstmt.executeQuery();
			while (rs.next()) {
				Student_p s = new Student_p(rs.getString(1), rs.getString(2), rs.getString(3), rs.getInt(4));
				slist.add(s);
			}
		} catch (SQLException e) {
			System.out.println("DB 에러:" + e.getMessage());
	...
	public List<Student_p> schStudent(Student_p sch) {
		List<Student_p> slist = new ArrayList<Student_p>();
	...
	// 성적 입력/수정 메서드
	public void udtStuLec(Student_p udt) {
	...
	// 성적 삭제[초기화] 메서드
	public void delGrade(Student_p del) {
		String sql = "UPDATE stdLecture\r\n"
				+ "SET attendance = 0,\r\n"
				+ "   midtest = 0,\r\n"
				+ "   endtest = 0,\r\n"
