...
import jspexp.project_4.vo.Grade;
import jspexp.project_4.vo.Lecture;
import jspexp.project_4.vo.Student;
...
public class UniversityDao {
	...
	// 학적정보조회
	public List<Student> schStdInform(String id) {
		...
		String sql = "SELECT s.Id,stdName,stdYear,enterYear,address"
				...
				+ "WHERE s.majorNo = m.majorNo and s.id=?";
		try {
			con = DB.con();
			...
			while (rs.next()) {
				list.add(new Student(rs.getString(1), rs.getString(2), rs.getInt(3), rs.getInt(4), rs.getString(5),
	...
	// 학적정보변경
	public int uptInform(Student upt) {
		int cnt = 0;
	...
	// 강의계획서조회
	public List<Lecture> schLecPlan(Lecture sch) {
		...
		try {
			...
			rs = pstmt.executeQuery();
			while (rs.next()) {
				list.add(new Lecture(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getInt(5),
						rs.getString(6)));
			}
			System.out.println("데이터 건수:" + list.size());
		} catch (SQLException e) {
	...
	// 강의평가조회
	public List<Lecture> schLecEval(String id) {
		List<Lecture> list = new ArrayList<Lecture>();
		...
		try {
			...
			rs = pstmt.executeQuery();
			while (rs.next()) {
				list.add(new Lecture(rs.getInt(1), rs.getString(2), rs.getInt(3), rs.getString(4), rs.getString(5),
						rs.getString(6)));
			}
			System.out.println("데이터 건수:" + list.size());
		} catch (SQLException e) {
