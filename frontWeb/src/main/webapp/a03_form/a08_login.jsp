...
<body>
	<%--
	클라이언트   <====>   서버
	1. 웹서버가 고정 아이피(도메인주소), 서버를 프로젝트단위 소스와 함께
		실행을 한 상태..(준비가 된 상황) - 마트에 손님 받을 준비
	2. 클라이언트 url 주소로 도메인 => ip 주소
		==> port(마트에 여러물건을 파트코너가 있듯이 웹서비스1,
		웹서비스2,.. 오라클서버)
		==> context path 설정에 의해서 서버안 소스를 접근(webap)
		/, 프로젝트명/
	...
	4. 클라이언트는 위 html랜더링한 화면을 브라웅저에 의해서 볼 수 있다.
	...
	6. 서버는 해당 자원을 경로따라서 받고, 요청값이 있는 경우는
		...
		ex) id=himan&pass=7777 ==> ${param.hi}
			==> DB서버접속 ==> 결과에 대한 최종 html을 만드는
			자바+html 코드를 통해서 ==> html만들어서 결과 코드를 
			클라이언트에 전달.
	...
	3. jsp 페이지를 form으로 만들고 submit 처리하면 다시 현재 페이지호출
		이때 다시 현재 페이지를 호출하여 처리가 가능하다.
		즉, 물리적으로는 하나의 페이지이지만, 시간 간격상 다른 두개의 페이지가
		처리되는 것이다.
		cf)  
			사람1 --(너 멋있어졌구나)--> 사람2
									이 말을 듣기 전 사람2의 정보와
									이 말을 들은 후 사람2의 정보는 다르다.
			client <===> 서버
		1. 브라우저에서 url주소로 a08_login.jsp페이지를 호출
		2. 서버는 a08_login.jsp 요청값이 없는 페이지 그대로 브라우저에서 전달한다.
			a08_login.jsp ==> @@@.html
		3. 브라우저에서 로그인 폼을 보고 아이디와 패스워드를 입력하여 전송(submit)버튼을 클릭한다.
			a08_login.jsp?id=himan&pass=7777
		4. 서버에서는 요청값을 받아서 해당내용을 실행시키고, 그 결과를 html를 만들어서 다시 
			client에 전송한다. ${param.id}
		5. 브라우저(클라이언트)에서는 이 내용을 최종 결과를 확인 할 수 있다.
	...
	<h3>로그인결과</h3>
	<h4>아이디:${param.id}</h4>
	<h4>패스워드:${param.pass}</h4>
	<h4>${param.id=='himan' and param.pass=='7777'?
			'로그인성공':'로그인실패'}</h4>
	<!-- 
