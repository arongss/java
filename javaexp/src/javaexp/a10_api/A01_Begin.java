	...
	public static void main(String[] args) {
		...
		3. 주요 클래스
			Object - 자바 클래스의 최상위 클래스로 사용
				...
				3) 그래서, 상위에 재정의하는 메서드는 상위 Object의
					참조 변수로 사용이 가능하지만
				4) 하위 고유의 메서드를 사용하고자 할 때는 typecasting
					이 필요로 하다.
					ex)
				...
				5) ArrayList list = new ArrayList();
					...
					list.add("안녕");
					list.add(new Man());
					이렇게 모든 객체를 할당할 수 있지만,
					해당 객체의 고유의 필드와 메서드를 사용하려면
					타입캐스팅이 필요로 하다.
					Person p = (Person)list.get(0)
					Man m = (Man)list.get(1);
				6) 위 같이 할당은 가능하지만 사용할려는 객체의 유형으로
					타입캐스팅하는 불편함이 있는데, 단일 객체를 사용할 경우
					Generic(<객체유형>)을 선언하는 경우 사용가능 하다.
		...
		5. 객체 해시코드(hashCode())
			...
			2) 논리적 동등 비교시 hashCode() 오버라이딩의 필요성
				컬렉션 프레임워크의 HashSet, HashMap,
				Hashtable과 같은 클래스는 두 객체가 동등한 
				객체인지 판단할 때 사용된다.
...
class Member{
	...
	public boolean equals(Object obj) { 
		// Objet obj = new Member("홍길동")
		// 참조값 instanceof 객체 : 두개의 객체가 같은 클래스를
		// 참조할 때, true
		boolean isEquals = false;
		Member m = (Member)obj;
		// 해당객체의 속성을 사용할려면 type casting이 필요.
