	...
	public static void main(String[] args) {
		...
		1. 자바프로그램에서 산술연산자
			...
			4) / : 나눗셈(자바만)
				...
				정수/(형변환)정수 : 소숫점이하 표식
				System.out.println(10/(double)3.0);  3.3333
				System.out.println((double)10.0/3);  3.3333
				System.out.println((double)10.0/(double)3.0);  3.3333
			5) % : 나머지값
				27%5 ==> 2
				11%2 ==> 1
...
//		System.out.println(""+num01 - num02); // 에러발생
		...
		es2) 현재 가지고 있는 금액이 100000일때, 위 사과를
			 구매하고 나머지 잔액
		ex3) 곰돌이 5마리가 빵을 23개를 먹을 때,
			 공평하게 나눈 빵의 갯수와 나머지 빵의 갯수
		변수 선언, 변수를 통해서 사칙연산자를 처리한 결과를 출력
		 * */
		int applePrice = 2000;
		int appleCnt = 5;
		int appleTot = applePrice*appleCnt;
		System.out.println("사과의 구매 총계:"+(applePrice * appleCnt));
		int money = 100000;
