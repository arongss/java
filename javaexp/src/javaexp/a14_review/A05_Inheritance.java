...
public class A05_Inheritance {
	...
	public static void main(String[] args) {
		...
		list.get(1).read(); // CookBook의 내용 처리
 ...
 */
// Main() 상위객체 = 하위객체
// 상위객체.메서드() : 하위객체의 재정의 된 내용을 처리된다.
// Worker w1 = new PoliceMan(); w1.working(); //다양한 하위 형태를 변형되어 처리
// Worier w2 = new FireMan(); w2.working();
/*
# 상속관계에 새로운 필요성
1. 상위객체는 주로 사용하지 않고, 하위 객체에 구체적인 기능메서드만 사용
	==> 싱위에 있는 메서드를 구체적할 필요 없음 ==> 추상적 메서드 처리
2. 재정의 하는 메서드명이 혹 실수로 다른 메서드를 사용하는 부분에 대하여, 컴파일로 엄격하게
	check할 필요성이 있음.
	==> 상위에 있는 재정의할 메서드를 추상메서드를 선언함으로 하위에 반드시 재정의 하게 함.
==> 위 내용을 다 처리하고, 다형성을 처리하는 새로운 solution이 생김 ==> 추상클래스

Larba vs Dron/Zerggling/Hydra를
