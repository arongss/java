...
2. 단계별로 이해
	1) 배열형 및 리스트 객체에 대한 이해
		...
		Emp e = list.get(0); indea번호로 하나씩 객체를 가져올 수 있다.
		for(Emp e:list){
...
# 추상클래스를 상속받은 하위 클래스의 내용을 다형성으로 처리하여,
1:다관계를 처리하는 경우..
1. 예제
	컴퓨터의 여러가지 부품을 Part라는 추상클래스를 상속받은 하위 
	실제 클래스로 생성하여 다양한 형태를 실제객체를 할당할 수 있는 
	구조로 처리하는 1:다 구조를 말한다.
2. 처리순서
	1) 추상 클래스 선언
	2) 상속받은 하위 클래스 선언 : 필요시 추가적으로 상속받은 하위클래스를 정의하여 
		처리하여야 한다.
...
class Cpu extends ComPart{
	...
	// abstract로 선언된 추상메서드는 반드시 재정의 하여 선언하여야 한다.
...
class Ram extends ComPart{
	...
	public void partFun() {
		// TODO Auto-generated method stub
	System.out.println(getKind()+"는 "+getPerFrm()+" 성능을 가지고 있다.");
	System.out.println("컴퓨터에서 휘발성 메모리를 처리해주는 부품이다.");
	}
}
class Ssd extends ComPart{
	...
	public void partFun() {
		// TODO Auto-generated method stub
	System.out.println(getKind()+"는 "+getPerFrm()+" 성능을 가지고 있다.");
	System.out.println("컴퓨터에서 하드웨어적으로 데이터를 저장해주는 저장 메모리 부품이다.");
	}	
}
