...
public class A07_Interface {
	...
	public static void main(String[] args) {
		...
		System.out.println("1. 바로 사용이 가능한 것(객체 없이 사용가능)");
		...
		FlyWay w3 = new Wing02(); w3.remoteWing(); w3.fly();
...
2. 인터페이스의 역할
	1) 개발 코드각 객체에 종속되지 않게 즉, 객체 교체할 수 있도록 하는 역할
	...
	4) 인터페이스의 구성 멤버
		interface 인터페이스명{
			...
			default 타입 메소드명(매개변수..){} 
				// 디폴트 메서드 - 공통기능 메서드  하위에서 객체의 참조값으로 사용
			static 타입 메소드명(매개변수){} 
				// 정적 메서드 - 인터페이스명.기능메서드() 형태로 사용.
		}
		# 선언하지 않더라도 자동으로 붙는 내용 : 컴파일시 아래 내용이 코드에 없더라도 
			자동으로 붙게 처리된다.
		- 일단 모든 멤버는 선언을 하지 않더라도 public이 자도으로 붙는다.
			public이 모든 멤버에 추가됨
		- 필드인 상수는 자동으로 static final이 붙는다.
			ex) ing PI = 3.14; ==> public static final int PI = 3.14;
				인터페이스명.PI 로 활용가능.
...
1. 인터페이스의 구성멤버인 4가지인 경우에 한정적으로 같편하게 사용할 경우에 사용되고,
...
interface FlyWay{
	...
	default void remoteWing() {
		System.out.println("원격 조정에 의해 날아간다(객체 공유)");
	}
	static void goMz() {
