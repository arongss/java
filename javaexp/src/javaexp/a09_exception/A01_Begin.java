...
1. 오류의 종류
	...
	2) 예외(Exception) *
		...
		에러가 나더라도 프로그램을 멈추지 않고 정상적인 처리를
		할 수 있게 해준다.
...
4. 예외 처리 코드
	...
	3) 예외 위임(throws)
		...
		예외를 위임하여 공통적으로 처리하는 곳에서 한번에 처리하는
		것을 말한다.
		메서드1() throws 예외클래스{
	...
	4) 강제 예외 처리(throw new 예외처리클래스())
		코드상 특정 조건이나 특정 위치에서 개발자가 사용자 정의
		또는 내장된 예외를 직접적으로 던지고, 해당 예외를 
		catch구문으로 처리하는 것을 말한다.
		try{
			if(cnt == 5){
