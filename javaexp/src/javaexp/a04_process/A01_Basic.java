...
public class A01_Basic {
	...
	public static void main(String[] args) {
		...
		# 코드 실행 흐름 제어
		1. 정상적인 코드 실행 흐름
			1) main() 메소드의 시작인 중괄호 {에서 끝
				중괄호 }까지 위 ==> 아래로 방향으로 실행된다.
				- 코드는 기본적으로 위에서 아래로,
				왼쪽에서 오른쪽으로 흐름 처리가 된다.
				- 단, 대입연산자를 통한 처리는 오른쪽에 있는
				데이터가 왼쪽에 변수로 할당 처리된다.
		2. 제어문의 역할
			코드 실행 흐름을 개발자가 원하는 방향으로 변경할 수
			있도록 도와준다.
		# 제어문의 종류
		1. 조건문
			if, switch문
		...
		# if문
		1. 조건문 결과 따라 중괄호{} 블럭을 실행할지 여부 결정할
			때, 사용한다.
		2. 조건식
		...
		* */
		...
		System.out.println("flow진행:"+(cnt++));
		/*
		ex) 과일의 갯수를 2개씩 증가시켜서 출력하기 위해 변수를 
		선언하여 처리할 때,
		...
		* */
		...
		System.out.println("곰돌이가 사과를 "+(appCnt+=2)+"개 먹었다..");
		if(appCnt==14)
			System.out.println("곰돌이가 사과를 "+(appCnt+=2)+"개 먹었다(바로밑)");
		System.out.println("곰돌이가 사과를 "+(appCnt+=2)+"개 먹었다..");
		System.out.println("곰돌이가 사과를 "+(appCnt+=2)+"개 먹었다..");
		System.out.println("곰돌이가 사과를 "+(appCnt+=2)+"개 먹었다..");
