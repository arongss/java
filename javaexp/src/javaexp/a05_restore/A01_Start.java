...
public class A01_Start {
	...
	public static void main(String[] args) {
		...
		/*
		# 우리가 방에 물건을 저장할 때, 효율적인 방법??
		1. 작은 물건? 큰물건
		2. 하나의 공간(작은 공간) - stack영역
			1) 작은 물건 - primitive data type(크기가 일정)
			2) 큰물건들이 있는 위치 정리 (해당 heap영역의 주소를 저장)
			 	- 크기가 일정치 않음 (객체형 데이터 - 배열, 내장 객체, 사용자 정의 객체)
			 	ex) 도서관/서점에서 물건 검색하는 단말기 : 해당도서의 위치가르켜 준다.
		...
		- 자바는 정수/실수/boolean형을 바로 선언하여 사용하는
			기본형 타입과, 객체형타입(배열/열거/클래스/인터페이스)에 의해
			특정한 메모리를 참조하는 참조타입으로 크게 분류할 
			수 있다.
		# 변수의 메모리 사용
		1. 기본 타입 변수 - 실제 값을 변수안에 저장
			cf) stack영역에 선언하고 실제 데이터를 할당
		2. 참조 타입 변수 - 주소를 통해 객체 참조
			cf) heap영역에 저장하고 이 heap영역의 주소값을
				stack영역에 저장한다.
		 * */
		int num01 = 15; // stack영역에 15할당.
		int num02 = 15; // ex) 여기에 바나나가 있습니다. 바나나를 위치 시킴
		int[] arr01 = {10}; // heap영역에 주소를 stack영역에 arr01 - 자동차가 있습니다. 자동차가 있는 위치 주차구역1 12자리
		int[] arr02 = {10}; // heap영역에 주소를 stack영역에 arr02
		// stack영역과 heap영역 할당..
		System.out.println(num01+":"+num02);
		// 1. 동일한 데이터이므로 동일한 데이터 할당.
		System.out.println(arr01+":"+arr02);
		// 2. 동일한 데이터이지만 heap영역의 주소값이
		//	   다르기에 다른 데이터가 나타난다.
		int num03 = num01; // num01의 데이터를 num03에 할당. ex) 여기에 바나나3가 있습니다. 바나나라고 저장된 실제바나나를 할당
		System.out.println(num01+":"+num03);
		num03 = 20; // 바나나3이라고 이름 붙인 곳에 바나나 주스를 위치 시킴..
		System.out.println(num01+":"+num03);
		// 객체의 경우..
		int[] arr03 = arr01;
		// 메모리의 주소를 할당하기	자동차3가 있습니다. 여기에 자동차가 있습니다. 라고 적힌 곳의 위치 정보를 복사해 할당.
		...
		// * 자바의 데이터의 비교 연산자는 stack영역의 데이터를 비교ㅏ는 것이다.
		// 1. 기본 데이터 유형 - 실제 데이터를 확인해서 true/false
		...
		// ex2) int train[]={3}을 3개 train1, train2, train3을
