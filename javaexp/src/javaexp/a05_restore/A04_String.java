...
public class A04_String {
	...
	public static void main(String[] args) {
		...
		1. 문자열을 저장하는 클래스 타입
			...
			스택영역에 subject이라는 이름으로 heap영역에 있는 "자바"문자열을 호출하여 사용
		...
		1. String name1 = "홍길동";
		   String name2 = "홍길동";
		   "홍길동"을 기준해서 heap영역 할당된 동일한 주소를 name1, name2에서 호출해서 사용
		...
		2. String name3 = new String("홍길동");
		   ...
		   cf) 객체는 new 라는 키워드를 이용하여 객체를 생성하면 heap영역에 다른 주소를 사용한다.
		   	   {} : 내부적으로 new 타입[] 생략
		   ex) name3 == name4 ==> false 같은 문자열이지만 new통해서 다른 주소에 할당되었기 때문..
		...
		# 실무적으로 코드로 직접적으로 대입하는 것보다,
		  ...
		  의해서 가져오는 문자열 데이터는 new String()형식으로
		  heap영역에 따로 메모리를 만들어 가져오는 경우가 많다.
		...
		// ex1) 문자열로 유효한(인증된) id, pass를 선언하고, Scanner를 통해서 입력된 id, pass를 통해
		//		== (주소값)을 비교한 경우와 equals()를 통해 비교한 처리결과가 어떻게 다린지 확인해보세요.
