...
//		Comp com = new Comp(); 추상클래스는 하위 클래스의 다형성을 위한 클래스이라서 혼자 객체 생성 안됨..
		...
		l02.attack();l02.move();
		l03.attack();l03.move();
		// ex) 추상Player(선수의 정보)분류,이름
		//		trainning()공통  play():재정의(추상)
...
abstract class Player{
	...
	public void trainning() {
		System.out.println(this.name+" 열심히 연습을 한다");
	}
...
abstract class Comp{ // 회사
	...
	public abstract void earnMoney(); // 어떻게 버는지는 하위 클래스에 반드시 재정의
}
...
1. 개념
	1) 실체들 같의 공통되는 특성을 추출한 것
		ex1) 새, 곤충, 물고기 ==> 동물(추상)
		ex2) 삼성, 현대, LG ==> 회사(추상)
	2) 자바의 추상?
		실체 클래스들의 공통되는 필드와 메소드 정의한 클래스
		추상 클래스는 실체 클래스늬 부모 클래스 역할(단독으로 객체 생성을 하지 못한다)
...
class Multal extends Larva{
	...
	public void attack() {
		// TODO Auto-generated method stub
		System.out.println(getKind()+"원거리 공중공격(공대지/공대공)을 한다.");
	}
}
