...
public class A01_Begin {
	...
	public static void main(String[] args) {
		...
		1. 현실 세계 :
			...
			2) 부모가 자식을 선택해서 물려줌
		2. 객체 지향 프로그램
			1) 자식(하위,파생) 클ㄹ스가 부모(상위) 클래스의 멤버를 물려받는 것
			2) 자식이 부모를 선택해 물려받음
			3) 상속 대상 : 부모의 필드와 메소드
		ex)
			class Parent{
		...
		1. 자식 클래스가 상속할 부모 클래스를 지정하는 키워드
			class Child extends Parent{}
		2. 자바는 단일 상속 - 부모 클래스를 여러 개 나열 불가하다
...
// ex) 아버지(모습이 통통하다, running() 빠른 속도로 뛴다), 아들(키가 크기, showAll()-부모 구성+키))
class Father{
...
//		System.out.println("성격:"+charact);		접근제어자가 private인 경우 처리하지 못함.
		System.out.println("성격:"+getShape()); //간접적으로 접근을 하여 처리한다.
	}
}
class Mother{
	String charac ="현명하다";
