...
//		int tot = num01 + 10; 초기화되지 않아서 에러를 발생
		...
		# 변수의 사용 범위
		1. 변수는 {}(중괄호)를 기준으로 상하위 계층관계를 가지고
			처리할 수 있다.
			반복문/조건문, 클래스/메서드에서 언급
		2. 즉, 상위단계에서 선언된 변수는 하위에서 호출하여 사용되고,
		3. 하위 단계에서 선언된 변수는 상위에서 선언 및 호출하지
			못한다.
		# primitive 데이터 타입
		1. 자바에서는 기본 stack영역에서 선언 및 호출, 할당되는
			기본 데이터 타입으로 정수, 실수, 문자, 논리형이 있다.
			ps) 자바는 크게 메모리를 stack, heap으로 나누어서
				사용한다.
				작은 메모리인 기본데이터 유형은 stack영역에
				선언과 저장은 동시에 처리하고 있고,
				이런 기본유형을 모아서 리스트한 배열이나,
				객체유형은 stack영역에는 heap영역의 주소를
				저장하고, 실제 배열/객체는 heap영역에 저장처리한다.
				
				
		2. 메모리의 최소 기억단위인 bit가 8개 모여서
			byte가 됨으로 시작되었다.
		...
		3. 정수형
			...
			char	2byte  0~65535
				...
				'0' ==> 문자
				 ...
				 int유형은 casting에 의해 문자한자로 변경이
				 될 수 있다.
				
			short	2byte
			int		4byte (정수형의 default 데이터 유형)
				253, 연산자가 들어가는 순간 int형으로
				promote 된다.
				
			long	8byte
					ex) 21억 4천 이상의 데이터인경우
...
//		num01 = 257; 범위가 넘어 에러 발생..
		...
		// 0.0<=Math.random()<1.0
		var ran = (int)(Math.random()*26+65);
		var ran1 = (int)(Math.random()*26+65);
		var ran2 = (int)(Math.random()*26+65);
		var ran3 = (int)(Math.random()*26+65);
		var ran4 = (int)(Math.random()*26+65);
		System.out.println(ran);
		System.out.println((char)ran1);
		System.out.println((char)ran2);
		System.out.println((char)ran3);
		...
		// 1. 문자는 ''사이에 한자를 입력하여, char로 할당할
		// 		수 있다.
		char ch03='B';
		// 2. 코드값을 확인하기 위해서는 정수형 데이터에 바로
		//		바로 할당하여 처리할 수 있다.
		int code03 = ch03;
		System.out.println(code03);
		// 3. 정수형 변수를 선언하고, 해당 정수의 코드값에 해당하는
		// 		유니코드를 확인할려면, casting 으로 확인할
		// 		수 있다.
		int code04 = 70;
		 ...
		 * */
		double d01 = 2.242;
		float f01 = 3.42F; // float 유형일 때, F를 붙여서
		// 사용한다.
		/*
		5. 논리형
			1) boolean
		...
		/*
		ex1) 초기화 선언 확인
		현재 나이를 변수로 선언하여, 초기화 되지 않을 때,
		10년 후 나이를 처리하고,
		초기화한 후, 10년 후 나이를 처리하고 출력하세요.
		ex2) 문자'Z','홍' 의 유니코드값을 확인하여
			출력하세요.
		ex3) 정수 데이터 100에 해당 하는 문자를 출력하세요.
		* */
		int age1;
//		int age110 = age1 + 10; 초기화되지 않을 때, 에러 발생
		int age=25;
		int age10 = age + 10;
		...
		age = 28;
		int ageTot = age + 10;
		System.out.println("십년후 나이:"+ageTot);
		
		char ch05 = 'Z';
		int code05 = ch05;
		System.out.println(code05);
		char ch06 = '홍';
		int code06 = ch06;
		System.out.println(code06);
		
		int code07 = 100;
		char ch07 = (char)code07;
