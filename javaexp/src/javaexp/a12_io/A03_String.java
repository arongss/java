...
public class A03_String {
	...
	public static void main(String[] args) {
		...
		2. 주요 메서드
			...
			int read(byte[] b, int off, int len) : 입력 스트림으로부터 len개 바이트
				만큼 읽고 매개값으로 주어진 바이트 배열 b[off]부터 len개까지 저장한다.
				그리고 실제로 읽은 바이트 수인 len개를 리턴한다. 만약 len개를 모두 읽지
				못하면 실제로 읽은 바이트 수를 리턴한다.
			void close() : 사용한 시스템 자원을 반납하고 입력 스트림을 닫는다.
		 * */
		// 1. 입력스트림 객체 System.in 통한 객체 생성
...
//			System.out.println((char)cod); // 문자 한자를 가지고 출력..
			int n = is.read(brr);
			// 	ex) byte []이라는 문자를 담을 가방을 들고,
			//		is.read()라는 문자를 입력해주는 마트에 가서
			//		문자를 가방에 담아서 처리하고.
			//		최종적으로 문자데이터가 byte [] 가방에 담겨져 있음.
		...
		//		나타내세요. A04_StringExp.java (영문으로만)
