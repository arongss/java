...
public class A05_ContructorParam {
	...
	public static void main(String[] args) {
		...
		4. 호출
			1) 객체 생성시 호출
				...
				Person p04 = new Person("신길동",25);
				선언된 데이터유형과 갯수에 맞게 생성하여야 한다.
		ex) House 클래스 선언 후, char, int, double, boolean
			으로 매개변수가 할당하게 하는 1개의 생성자를 선언하고
			해당 생성자를 통해 객체가 생성되게 처리하세요.
			필드 (위 데이터 유형에 맞게 선언 및 생성자를 통해 초기화)
			
		 * */
		House h1 = new House('A');
		//.out.println("집의 등급:"+h1.grade);
		House h2 = new House((byte)25);
		// 25는 매개변수로 넘길 때는 정확하게 해당 유형으로
		// casting해서 전달하여야 한다.
