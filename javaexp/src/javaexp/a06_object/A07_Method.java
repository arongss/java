...
public class A07_Method {
	...
	public static void main(String[] args) {
		...
		1. 핵심 기본 유형
			...
			1) 리턴타입 :
				이 메서드를 호출했을 때, 나오는 데이터의
				유형을 선언한다.
				마지막에 리턴할 데이터의 유형을 선언한다.
				리턴값이 없는 경우에는 void를 적고 리턴하지
				않는다..
			...
			2) 매개변수 :
				메서드도 생성자와 동일하게 여러 개의 데이터를
				단일 또는 다중으로 입력받아서 처리할 수 있다.
				cf) 메서드 오버로딩
				메서드 이름이 같지만 매개변수의 갯수, 유형, 
				순서(유형이 다른)에 따라 선언을 할 수 있다.
	...
	//		main()에서 저장과 호출하여 출력하세요..
		...
		//		1) 기능메서드1 num01 저장
		//		2) 기능메서드2 num02 저장
		//		3) 기능메서드3 num01, num02 - 1/3오버로딩
		//		4) 기능메서드4 두개의 변수를 더해서 return
		//		5) 기능메서드5 두개의 변수를 곱해서 return
		Calcu c1 = new Calcu();
		c1.setNum01(10);
		c1.setNum02(20);
		System.out.println(c1.getPlus());
		System.out.println(c1.getMulti());
		c1.setNum01(20, 30); // 오버로딩
		System.out.println(c1.getPlus());
		System.out.println(c1.getMulti());
		System.out.println("생성자에 의해 초기화");
		Calcu c2 = new Calcu(20, 50);
		System.out.println(c2.getPlus());
		System.out.println(c2.getMulti());
	}
}
class Calcu{
	int num01;
...
class Person{
	...
	void setData(String name) {
		this.setData(); // 이 객체가 소속된 기능메서드 호출.
		this.name=name;
	}
