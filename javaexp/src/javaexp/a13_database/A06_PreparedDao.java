...
public class A06_PreparedDao {
	...
	public List<EmpQua> getEmpQuaList(EmpQua sch){
		...
		String sql = "SELECT empno, ename, qm.qu||'/4' qu, sal -- 사원번호, 사원명, 분기, 급여\r\n"
				+ "FROM emp e, (\r\n"
		...
		3. 조건문에 따른 내용은 sql 조건도 처리하고
			...
			if(sch.getEname()!=null) pstmt.setString(++cnt, ...) ==> 고정적으로 할수 없어서
			if(sch.getJob()!=null) pstmt.setString(++cnt, ...)		변수를 사용하게 된다.
			
		4. pstmt부분도 조건 처리하여야 한다.
		5. 특히, 동적 sql이 보갑해질 때는 순서 index부분도 변수를 선언하여 처리하여야 한다.
			int cnt = 1;
		...
		try {
			...
			rs = pstmt.executeQuery(); // sql을 넣지 않는다.!!(주의)
			// EmpQua(int empno, String ename, String qu, double sal)
			while(rs.next()) {
	...
	public static void main(String[] args) {
		...
		//dao.insertDept(new Dept(11,"총무","부산")); //등록
		// 10 ==> 55, 인사, 부산
		//dao.updateDept(10, new Dept(55, "인사", "부산")); //수정
//		dao.deleteDept(55);// 삭제
