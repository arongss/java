...
public class A01_Begin {
	...
	public static void main(String[] args) {
		...
		3. 객체들을 효율적으로 추가, 삭제, 검색할 수 있도록
			제공되는 컬렉션 라이브러리
		4. java.util 패키지에 포함
		5. 인터페이스를 토애서 정형화된 방법으로 다양한 컬렉션
			클래스 이용
		# 컬렉션 프레임워크의 주요 인터페이스
		1. 인터페이스
		...
		buyList.remove(0);
		System.out.println("크기:"+buyList.size());
		buyList.clear(); // 모두다 삭제..
		System.out.println("크기:"+buyList.size());
		System.out.println("비워있는지?:"+buyList.isEmpty());
		
		// ex) Member(이름/나이) class 선언, mlist로
		//		회원 3명을 등록하고, 마지막데이터 삭제,
		//		회원 목록 확인, 전체 삭제, 크기확인
		List<Member> mlist = new ArrayList<Member>();
		mlist.add(new Member("홍길동",25));
		mlist.add(new Member("김길동",21));
		mlist.add(new Member("신길동",29));
