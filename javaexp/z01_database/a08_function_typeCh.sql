...
2. 형변환 유형
	1) 묵시적 데이터 타입의 변환
		...
		경우가 있는데 이것은 오라클 시스템 내부적으로 타입을 변환했기 때문이다.
		ex) select *
			from emp
			where sal>='2000';
		문자열로 '2000'을 입력하더라도 시스템에서 연산을 인식하여 숫자로 자동형
		변환 시켜 처리된다.
	2) 명시적 데이터 타입의 변환
		...
		ex) 현재 컬럼 데이터 유형을 변환해서 조건에 맞게 처리하는 경우
			...
			해당 컬럼에 맞게 테이터 유형을 변환해서 처리하는 경우
				where hiredate = to_date('2000-01-12','YYYY-MM-DD')
				insert emp(hiredate) values(to_date('2000-01-12','YYYY-MM-DD'))
...
SELECT e.*, sal + to_number('2000') "합", e.*
FROM emp e
...
3. 출력할 형식(날짜)
	...
	7) DDD/DD/D : 날짜 표기
	8) DY/DAY : 요일 표기
	위의 있는 형식에 한글 등 트기수문자를 혼합해서 표기할 때는
	'YYYY"년도" MM"월" ' 형식으로 "추가문자열"을 표현한다.
 * */
SELECT ename, hiredate, to_char(hiredate,
	'CC"세기" AD YYYY/MM/DD Q"/4분기" MM"월" W"째주" DAY') show,
...
FROM emp;
-- ex1) 사원명는 @@년 @@월에 @@/4 분기 입사했습니다.
SELECT ename||'는 '||to_char(hiredate,'YY"년 " MM"월에 " Q"/4분기"')||' 입사했습니다.' show
FROM emp;
...
-- ex) sysdate를 활용해서 현재 시간을 @@시 @@분 @@초 (24시) 표시형태로
--		출력하세요.
SELECT to_char(sysdate, 'HH24"시 "MI"분 "SS"초"') 현재시간
FROM dual;
/*
