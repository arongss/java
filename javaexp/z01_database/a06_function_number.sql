	...
	4) floor(데이터) : 해당 데이터를 정수형으로 내림 처리한다.
		ex) mod(), /, *, floor/ceil : 중첩함수를 이용해서 다양한 데이터의 범위와
			...
			mod() : 반복된 조건의 데이터를 처리할 때, 사용
				홀짝, 3의배수(0,1,2), 5의 배수...
	ps) 날짜형의 경우에서 중첩함수로 위 숫자형 데이터에 대한 절삭이나 올림을
		처리하는 경우가 많다. 
		ex) 날짜 + 1 : 1일,  날짜 + 0.5 : 1일12시간
			add_months(날짜,1) 1달 이후, add_months(날짜,0.5) 15일이후
...
SELECT ename, sal, round(sal,-2) 급여1, trunc(sal+nvl(comm,0),-3) 급여2
FROM emp;
-- ex2) 급여의 1/13을 나누어 소숫점 2자리 절삭, 정수형올림 처리로 출력하세요
SELECT ename, sal, trunc(sal/13,2) 급여1, ceil(sal/13) 급여2
FROM emp;
/*
# mod
