...
3. 트랜잭션
	...
	이 때, COMMIT하는 순간 현재 클라아언뿐만아니라 다른 계정으로 접속하더라도 변경되게 처리된다.
# 데이터 입력 처리
...
--		부서별 최고 급여와 급여를 subquery를 통해 위 emp13테이블에 입력하세요.
DROP TABLE emp13;
-- 0. 위 선언 구조에 의한 데이터가 없는 테이블 생성.
CREATE TABLE emp13
