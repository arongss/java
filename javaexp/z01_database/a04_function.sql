...
2. sql 함수의 유형
	...
	2) 복수 행 함수 : 조건에 따라 여러 행을 그룹화하여 그룹별로 결과를 
		하나씩 변환하는 함수
		여러 행의 데이터를 함수를 적용했을 때, 단일 또는 줄어든 결과를
		리턴하여 처리할 때, 사용된다
		ex) 부서별 최소급여
		10 3000
...
--		[컬럼]
SELECT 'The job of '||initcap(ename) ||' is a/an '||initcap(job)||'!!!' show FROM emp;
/*
# lower(), upper()의 사용
1. 입력되어 있는 데이터를 대소문자 상관없이 검색을 처리할 때, 활용
2. 'a'문자로 입력하고나 'A'를 입력해서 검색하더라도 대소문자 상관없이
...
--		있을 지라도 대소문자 관계없이 키워드 검색되게 할려면 사용하는 sql을
--		처리하세요.
...
SELECT 'hello' 인사1, length('hello') 인사1크기1, lengthb('hello') 인사1크기2,
		'안녕' 인사2, length('안녕') 인사2크기1, lengthb('안녕') 인사2크기2
	FROM dual;
SELECT ename, length(ename) 사원명길이
FROM emp;
-- ex) 사원 테이블에서 사원명과 직책의 글자 수를 표현하되, 직책이 6이상인 데이터만
--		출력하세요
...
2. substr(문자열데이터, 시작위치, 시작위치의 갯수) :
	문자열 데이터를 시작위치와 마지막위치를 기준으로 절삭처리하여 사용하는 것을 
	말한다.
