...
# subquery
1. 개요
	하나의 sql 명령문의 결과를 다른 sql명령문에 전달하기 위해 두 개 이상의
...
1. 단일행 서버 쿼리
	...
	ex) select *
		...
		부서번호과 10일 사원의 최대 급여보다 적은 급여를 가진 사원 정보
...
-- 부서별 최고 급여자의 사원정보(다중행/다중열로 검색 처리)
...
-- ex2) 급여가 2000~3000 사이의 관리자정보와 부서정보와 일치하는 사원을 출력(다중행)
SELECT *
FROM emp 
WHERE (mgr, deptno) in(
	SELECT mgr, deptno
...
SELECT (SELECT max(sal) 
		FROM emp 
		WHERE deptno=e.deptno) "부서별최고급여", e.*
FROM emp e;