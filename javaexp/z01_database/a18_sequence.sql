...
6. 선언된 sequence 사용
	...
	2) 시퀀스명.curval : 현재 시퀀스번호 출력
 * */
/*
# sequence의 목적
1. 테이블에 고정형 데이터형식의 primary key를
	사용할 때, 가장 많이 활용된다.
	...
	create sequence seq_01
		...
		insert into board01 values(seq_01.nextval, '내용'...);
		insert into board01 values(seq_01.nextval, '내용'...);
2. 사원번호등 합성적인 코드에 sequence 를 만들 때.
	단과코드+입학년도+학과코드+일련번호
	H01P040 1000~~
		stud_seq
...
5. alter sequence 시퀀스명
	start with n : 외에 수정이 가증. 
	- start with를 바꿀려면 drop시키고 다시 생성하여 start with를 지정 처리..
 * */
...
-- 위 시퀀스 생성후 seq_01.nextval로 확인 처리
SELECT * FROM emp02;
INSERT INTO emp01(empno) values(seq_02.nextval);
