 ...
 * */
SELECT * FROM emp;
SELECT * FROM dept; -- deptno 로 연관 관계가 설정
-- 7369 사원의 부서정보 가져오기
SELECT *
FROM emp 
WHERE empno = 7369;
...
# equi join
1. 개념
	1) 조인 대상 테이블에서 공통 컬럼을 '=' equal 비교를 통해 같은 값을
...
SELECT ename, sal, dname, e.deptno -- 공통컬럼만 alias이름 선언하다
FROM emp e, dept d
WHERE e.deptno = d.deptno 
AND sal >= 3000;
SELECT *
FROM EMP e, DEPT d 
WHERE e.deptno = d.deptno;
-- 조인을 하면 해당 조인된 구문에 의해 나타날 데이터를 테이블형식으로 그동안 진행한
-- 여러가지 함수, 그룹데이터, 조건문 처리를 할 수 있다.
SELECT * FROM emp;
...
/*
# Non equi join
1. 조인하는 두개의 테이블의 값이 동일하지 않고, 범위로서 조인하는 경우를 말한다.
 * */
...
1. equi join의 조인 조건에서 양 테이블의 컬럼 값 중, 어느 하나라도 null이면
	...
	ex) 사원정보와 부서정보 테이블 조인에서 부서번호가 40인 사원정보는 없기에 
		부서정보 40은 출력되지 앟게 된다.
		수강생과 교수 테이블에서 수강신청을 하지 않은 교수의 데이터를 출력되지 않는다.
2. equi join에서 양측 컬럼 값 중의 하나가 null 이지만, 조인 결과로 출력할 필요가 
	있을 때, outer join 사용한다.
...
FROM EMP e, DEPT d 
WHERE e.deptno(+) = d.deptno;
-- 부서테이블에 40 부서정보와 연결되는 정보가 없어서, equi join으로는 보이지
...
FROM EMP e, DEPT d 
WHERE e.deptno(+) = d.deptno;
-- max(nvl(sal,9)) : matching 되는 데이터가 없기에 통계치도 null이 나오는
-- 것을 0으로 처리해준다.
...
WHERE e.deptno(+) = d.deptno
AND sal >= 3000;
SELECT dname, nvl(ename,'엾음') ename, nvl(sal,0) sal 
FROM emp e, dept d
WHERE e.deptno(+) = d.deptno
AND sal < 3000;
