...
1. 특정한 문자열을 검색해서 해당 문자열의 위치를 index로 반환처리 하는 함수
	ex) 주로 대용량의 문자열 데이터를 검색해서 해당 문자열의 첫번째
	위치를 index로 표현해준다.
2. 추가옵션
...
FROM dual;
SELECT * FROM emp;
-- ex) 직책을 검색하되 MAN이 있는 것을 검색하되 해당 위치를 이름, 직책, 검색위치
--		로 출력하세요
...
2. 형식
	lpad(데이터, 크기, 덧붙일문자열)
	rpad(데이터, 크기, 덧붙일문자열)
ps) 특정한 컬럼을 동일한 자리수를 만들거나, 가변형데이터를 고정형데이터로 변경할 때,
	주로 사용된다.
	varchar2(10) ==> char(10)
	최대크기			 모든크기
	himan==> 5b		 himan     ==> 10b
...
FROM emp;
SELECT lpad(ename, 8, '#') 사원명, rpad(job, 9, '@') 직책명
FROM emp;
-- 중첩 함수에 대한 연습
-- 1. 기본 함수 기능 충분히 연습
-- 2. 이전에 했는 함수를 통해 처리할 수 있는 데이터를 상기 및 의문
-- 3. 다양한 문제를 통해서 필요로 하는 중첩함수 처리..
/*
# ltrim/rtrim
1. trim은 절삭이라는 말로, 오른쪽 또는 왼쪽에 있는 특정한 문자열을 삭제 처리할 때 
	사용된다.
...
cf) char 고정형 데이터를 ==> varchar2 가변형 데이터로 변환하여 사용할 때,
	주로 사용된다.
ex) 웹에서 등록하거나 조회할 때, 자기도 모르게 space를 입력해서 공백이 들어간 경우에
	이를 통해 입력된 데이터를 같은 문자가 있더라도 검색이 안되는 경우가 많다.
	이 때, 프로그램이나 sql 처리시 trim()을 해서 등록 또는 검색하면 이러한 데이터가
	정상적으로 처리된다.
	id : [himan ]
	pass : [ *****] 
 * */
...
SELECT job, ltrim(job,'S') job2, rtrim(job,'N') job3,
