...
SELECT sal, ename
FROM emp
ORDER BY sal desc; -- sal를 기준으로 데이터를 정렬 처리
/*
# where 조건문
1. 질의어에서 필요한 관계대수(=, >=, <=, >, <, !=, <>)로 select의 컬럼에 대하여
	행단위로 조건에 해당하는 데이터를 로딩해준다.
2. 관계대수를 연결하는 논리처리(and, or, not, subquery(중첩정의))를 통해서 행단위 데이터를
	필터링한다.
 * */
...
FROM EMP e 
WHERE sal >=2000;
-- ex3) 연봉이 1000~3000이고 부서번호가 10인 사원의 사원번호, 사원명, 급여, 부서번호를
--		출력하세요.
...
/*
# 테이블에 ALIAS를 사용하는 경우
1. 두 개 이상의 테이블을 사용하는데, 두개 테이블에 공통 컬럼이 있을 경우
...
SELECT *
FROM emp 
WHERE deptno <> 20; -- <> 도 같지 않을 때에 사용된다.
SELECT *
FROM emp 
...
-- 부서번호가 20이고 30인 경우가 아닐 때 부서번호가 20 이 아니거나 30이 아닌 경우
...
-- ex3) 급여가 (1000 미만 이거나 4000 이상)이 아닌 경우를 출력하세요.
SELECT *
FROM emp 
...
-- ex) empno가 7600~7900 사이의 사원정보 출력..
SELECT *
FROM emp 
