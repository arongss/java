...
1. 데이터가 할당되지 않는 경우를 의미한다.
	...
	주의) 눈으로 데이터가 없는 경우와 ''이 들어간 경우가 차이가 나지 않는 경우가
	있다. ''은 공백데이터가 입력된 경우고 null은 데이터 자체가 없는 경우를 말한다.
2. 처리 내용
	1) 조건문 처리 where
	...
	2. nvl을 활용해서 null데이터에 대한 처리를 할 때, 해당 유형에 맞게 ''(문자열),
		0(숫자형)으로 처리하여야 한다.
		ex) nvl(ename, 0) (X)
			nvl(comm, '') (X)
...
-- ex) 보너스가 0인 경우와 null인 경우를 제외한 경우 사원명과 급여, 보너스, 급여+보너스
--		를 출력하세요.
...
3. 이러한 중복을 제거해주는 명령어가 distinct 이다.
...
FROM emp
ORDER BY deptno;
-- distinct를 사용하면 두개 모두 다 비교하여 다른 것을 filtering한다.
/*
# in 구문
1. 데이터는 or 조건에 관련된 내용은 해당 조건 중에 참이 하나라도 있으면
...
-- ex) 직책이 'SALESMAN'이거나 'MANAGER'인 경우에 사원 정보를 출력하세요.
SELECT ename, job 
FROM emp
...
ex) 사원명에 'A'라는 문자열을 포함하면 조회되게 한다.
	...
	ex) 글자수4자리이고, 2번째자리에 A가 포함
		like '_A__'
 * */
SELECT * FROM emp WHERE ename LIKE '%A%';
SELECT * FROM emp WHERE ename LIKE 'A%';
SELECT * FROM emp WHERE ename LIKE '%N'; -- N으로 끝나면 검색
SELECT * FROM emp WHERE ename LIKE '__A%'; -- 3번째 자리가 A이면 검색
-- ex1) 직책에 'MAN'이라는 글자가 포함되어 있으면 조회되게 하세요
SELECT * FROM emp WHERE job LIKE '%MAN%';
-- ex2) 사원명에 S로 끝나는 글자 데이터
...
5. 기본 형식
	...
	order by 컬럼1명 [asc/desc], 컬럼2 [/desc],...
