...
SELECT job, count(job) 사원수, round(avg(sal)) 평균급여
FROM emp 
GROUP BY job;
-- 날짜형 데이터 min, max()는 최근 데이터, 가장 먼저 데이터
SELECT min(hiredate) "가장 먼저", max(hiredate) "가장 나중"
FROM emp;
...
GROUP BY deptno
HAVING avg(sal)>=2000; 
-- 적용한 그룹함수의 조건을 처리할 때는 having을 활용한다.
...
SELECT to_char(hiredate,'YYYY') 연도, count(to_char(hiredate,'YYYY')) 사원수, max(sal) 최대급여
FROM emp 
