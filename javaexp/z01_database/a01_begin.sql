/*
# 전날 sql 파일 처리
1. 파일에 내용 적고, 실행하는 것을 기존에 연결 상태로 계속할 수 있다.
2. 저장 누르는 순간 연결은 끊어 진다.
...
emp 테이블의
	empno ==> eno, ename ==> name, mgr ==> management,
	sal ==> salary
 * */
SELECT empno AS eno, ename name, mgr management, 
		sal AS salary
FROM emp;
/*
...
SELECT empno+10000 "사원번호(만단위)",
	...
	sal+comm "합산1(sal+comm)",
	sal+nvl(comm,0) "합산2(sal+comm)"
FROM emp;
-- comm값이 없는 경우 합산하면 null된다.
-- 연산을 처리하는 경우 피연산자가 하나라도 데이터가 null인 경우 연산이
-- 되지 않고 null로 처리가 된다.
-- nvl(컬럼, null인경우 초기값) null인 경우 특정한 값으로 초기화
...
-- # 사원 정보/부서 정보 #
-- 사원명 @@@의 사원번호는 @@@ 입니다.
SELECT '사원명 '||ename ||'의 사원번호는 '||empno||' 입니다.' "# 사원 정보 #"
FROM emp;
