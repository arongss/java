...
FROM dual;
-- ex) 분기별 보너스를 1사분기 100%, 2사분기 200%, 3사분기 300%, 4사분기 400%로
--		처리하기로 했다. 사원명, 급여, 보너스를 출력하세요.
...
SELECT floor(to_date('2022-12-31', 'YYYY-MM-DD')-sysdate) "남은날수"
FROM dual;
-- ex2) hiredate에서 입사년도만 현재년도로 바꾸어서 날짜 데이터를 만든 후 사원명, 입사일
--		을 출력하세요
SELECT ename, 
	to_char(sysdate,'YYYY') "현재연도",
	to_char(hiredate, 'MMDD') "입사월일",
	to_date(to_char(sysdate, 'YYYY')||to_char(hiredate, 'MMDD'),'YYYYMMDD') "신입사원"
FROM emp;
/*
...
1. 가장 내부에 있는 함수 f1의 결과값을 다음 함수 f2에 인수로 적용하고, 
	...
	- 가장 내부에 있는 함수부터 적용하여 리턴한 값을
		외부 함수의 매개변수로 적용하여 처리된다.
 * */
/*
# 일반함수
1. nvl(데이터, null값일 때 처리할 데이터)
	해당 데이터가 null값일 때, 처리할 데이터를 설정함으로 null에 대한 연산 처리
	를 하는 것을 말한다.
 * */
...
동일하지 않으면 첫버째 데이터1을 반환하는 함수를 말한다.
 * */
...
-----------------------------
-- ex) 보너스를 600으로 일괄 지급하기로 했는데, 급여의 50%로 100단위 절삭과 동일하면
--		600 그대로 지급, 그렇지 않으면 comm으로 지급 처리하자.
...
--		사원명 사원번호 구분자 팀
SELECT ename, empno, mod(empno,2) 구분자, decode(mod(empno,2),0,'홍','청') 팀
FROM emp;
-- 실무예제 decode(mod()) : 숫자형데이터에서 특정단위로 구분하여 데이터를 처리할 때
-- 		둘중에 하나일 때, 셋중에 하나일 때는 나머지 값을 기준으로 처리..
-- 		floor(), ceil()
...
decode(job,'CLERK','50%','SALESMAN','80%','PRESIDENT','100%','MANAGER','120%','ANALYST','150%','해당사항없음') "보너스%",
sal*decode(job,'CLERK',0.5,'SALESMAN',0.8,'PRESIDENT',1.0,'MANAGER',1.2,'ANALYST',1.5,0.0) "보너스"
FROM emp;
...
1. decode의 확장된 함수 형식을 표현식 또는 컬럼값 '=' 비교를 통해
	...
	case함수에서는 산술연산, 관계 연산, 논리 연산을 통해서 boolean,
