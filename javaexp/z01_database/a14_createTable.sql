...
1. 기본 형식
	...
	ex) 회원테이블
	create table member(
		id varchar2(20) primary key, -- 중복 및 null
		pass varchar2(20) not null, -- 반드시 입력되어야 됨
		name varchar2(50) nou null,
		point number,
...
--		물건의일련번호(char(8)),물건명(varchar2(50)),
...
INSERT INTO product values('PRD10002','딸기',12000,'딸기는 1box단위...');
SELECT * FROM MEMBER;
-- 1:다 관계의 연관관계 테이블 생성
-- 특정한 회원이 구매한 물건 : buyproduct
-- 속성 도출 : 구매한회원아이디, 구매할물건아이디, 구매할갯수, 구매한 날
/*
# 테이블의 데이터 입력 형식(DML)
1. 기본 형식
	1) 전체 데이터 입력 : 생성된 컬럼 순위로 데이터 유형에 맞게 입력
...
1. 복사테이블 만들기
	1) 데이터 + 기본 속성
		...
		as select empno no,
				  ...
				  from emp;
		: sql명령문에 의해서 컬럼의 속성과 컬럼명이 결정되어 테이블이 만들어 진다.
...
/*
# 복사테이블의 한계
1. 무결성 제약을 위한 comstraints는 복사가 되지 않는다.
...
INSERT INTO emp10 values(8001,'마길동','대리',7782,to_date('2022-01-10','YYYY-MM-DD'),4000,200,20);
-- 부분 컬럼 입력
...
3. erd 모델링
	...
	2) 연관 관걔 설정
		...
		==> 추후 확장성 유무에 따라 식별관계/비식별관계로 설정하는 기준으로
			처리하면 된다.
	3) 논리설계/물리설계
...
SELECT * FROM MEMBER;
SELECT * FROM product;
-- ex1) join 관계에 의해 himan이 구매한 물건명, 가격, 갯수를 출력하세요.
SELECT m.id, m.name "구매자명", p.name, 
		price, cnt, price*cnt "총계" -- 총계는 유도 속성에 의해 만들어 진다.
