...
SELECT sysdate 현재, sysdate + 1 "1일후", sysdate - 1 "1일전",
		sysdate + (100/24) "10시간 이후"
FROM dual;
SELECT ename, hiredate, hiredate-7 "입사 7일전", hiredate+100 "입사100일후"
FROM emp;
-- ex) 사원명과 함께 인턴기간(입사후, 120일), 입사 8일전, 
--		현재일 기준으로 근무 일수(정수로 절삭) 출력하세요.
--		sysdate : 현재일, floor()
...
2. 기본 형식
	...
	- 소숫점 이하의 데이터가 나오는 이유는 1이 개월단위이기에 일은 1/30..
	시간은 1/30/24 등으로 연산을 하기 때문이다.
	주의) 월 30일, 31, 29 등 월단위의 개월 수 처리시, 데이터 처리에 
		변수가 있는데, 이것 때믄에 기능함수를 이용하여야지 정확히 계산된다.
...
SELECT sysdate, sysdate+100, 
		floor(months_between(sysdate+100, sysdate)) "100일 이후 개월 수"
FROM dual;
...
/*
# 개월 수 추가 add_months
1. 특정한 날짜를 기준으로 개월 수를 추가하여, 해당 이후 날짜는 해당 이전 개월수에 
	날짜를 처리할 때, 사용된다.
2. 형식
...
FROM dual;
-- ex) 입사 후, 토요일 여행가기로 했다. 해당 날짜를 사원명과 함께 출력.
