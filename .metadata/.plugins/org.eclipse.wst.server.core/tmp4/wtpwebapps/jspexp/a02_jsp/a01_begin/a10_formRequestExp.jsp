...
# ex) 영화명, 가격, 관람인원을 입력받아서 영화명과
		총관람비용을 하단 테이블에 출력하게 하세요.
--%>
<h2>영화 관람</h2>
<form>
	영화명:<input type="text" name="mname"/>
	가격:<select name="pirce">
...
// 1. 초기로딩된 페이지와
// 2. 요청값을 전달후 만들어진 페이지는 다른 모양으로 나오는
// 초기화면과 요청값을 받은 후 나오는 2개의 논리적 페이지를 포함하고 있다.
	...
	if(mname!=null){
		...
		if(pirce==null) pirce="0";
		String cnt = request.getParameter("cnt");
		if(cnt==null) cnt="0";
		// 가져온 price, cnt를 숫자로 형변환 후, 연산 처리(*) tot 할당.
		int tot = Integer.parseInt(pirce)*Integer.parseInt(cnt);
	%>
	<table>
		<col width="40%">
		<tr><th>영화명</th><td><%=mname %></td></tr>
