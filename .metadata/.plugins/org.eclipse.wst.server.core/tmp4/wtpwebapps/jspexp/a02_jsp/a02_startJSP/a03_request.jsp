...
3. request 기본 객체 - 주요 정보 제공 메소드
	...
	4) getContextPath() : JSP 페이지가 속한 웹 어플리케이션의 컨텍스트 경로를 구한다.
		...
		<script src = "<%=path%>a00_com/a01_com.js">
		<img src="<%=path%>a01_img/img01.jpg">
		<a href="<%=path%>a01_main/menu01.jsp">
		
		context명을 서버에서 /로 설정했기때문에 사용하지 않고 바로 처리함
		<link href="/a00_com/a01_common.css" rel="stylesheet">
...
2. 기능메서드
	...
	request.getParameterMap() : 웹브라우저가 전송한 파라미터의 맴을 구한다. 맵은
		파라미터 이름, 값 쌍으로 구성된다.
	==> 요청 key를 모르거나, 변동이 되는 경우, 해당 요청 key와 값을 설정하여 처리할
		때 활용된다.
 --%>
	<%--
	http://localhost:7080/jspexp/a02_jsp/a02_startJSP/a03_request.jsp
	?name=홍길동&age=25&loc=서울&price=3000&price=4000&price=5
	 --%>
	<h2>name:<%=request.getParameter("name") %></h2>
	<h2>name:<%=request.getParameter("age") %></h2>
	<h2>name:<%=request.getParameter("loc") %></h2>
	<%
	String prices[] = request.getParameterValues("price");
	if(prices!=null){
	%>
	<h2>price[0]:<%=request.getParameterValues("price")[0] %></h2>
	<h2>price[1]:<%=request.getParameterValues("price")[1] %></h2>
	<h2>price[2]:<%=request.getParameterValues("price")[2] %></h2>
	<%} %>
