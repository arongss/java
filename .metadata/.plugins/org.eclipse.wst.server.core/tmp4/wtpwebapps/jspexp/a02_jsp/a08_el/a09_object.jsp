...
2. 아래의 방법에 의해 객체를 저장하고 처리한다.
	1) 객체 저장
		...
		<c:set var="nam01" value="${키}"/>
	2) 바로 호출 및 할당
		<c:set var="nam02" value="<%=new Person("홍길동",25,"서울")%>"
...
<%--
	p02.setName("신길동");
# property
1. 객체단위로 접근가능한 public 선언된 메서드를 호출할 때, 사용하는 개념이다.
2. getXXX(), setXXX() 메서드로 property라고 한다.
3. property 명의 기준
	get/set메서드에서 get/set을 삭제하고 대문자로 시작하는 메서드명을 
	소문자로 변경하여 해당 내용을 property지칭한다.
...
<h3>사원명:${emp.ename}</h3>
<%-- emp.setEname("홍길동")
ex) Dept 를 선언하고, 위 형식으로 부서번호 부서명 부서이름을 할당하고
 --%>
