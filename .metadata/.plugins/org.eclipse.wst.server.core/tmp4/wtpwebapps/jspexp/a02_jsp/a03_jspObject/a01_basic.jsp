...
2. 복잡한 if-else 사용시, out 기본 객체 사용하면 편리하다.
	1) scriptlet, expresion의 복잡한 사용보다. out으로 활용할 때
		더 효율적일 때가 많다.
3. 출력 메서드
...
<%--
ex) cnt를 1~16까지하여 번호가 있는 테이블(4X4)을 만들어 보세요.
	(조건문 쓰기 곤란한 분들 이중 for문 쓰셔도 됩니다)
 --%>
<h2>테이블1</h2>
<table>
<%
...
부서정보:<select name="dname" onchange="alert(this.value)">
<%
...
<%-- ex2-1) 사원번호와 사원명을 배열로 선언하고,
		select로 선언하여 value에서 사원번호 레이블에는 사원명으로 처리되게 하세요 --%>
<%
	int empnos[] = {7780,7781,7782,7783,7784};
	String enames[] = {"홍길동","김길동","신길동","마길동","오길동"};
%>
<h2>사원정보</h2>
...
<%--ex3) h3로 7가지 무지색 배경색상과 함께 출력하기(무지개 배경생상-배열선언) --%>
<%
...
<%
	for(int cel=1;cel<=6;cel++){
		...
		out.print("<td style='background:"+(cel%2==0?"pink":"")+"'>"+cel*3+"</td>");
		if(cel%3==0) out.print("</tr>");
	}
%>
</table>
<%--ex5) table로 타이틀로 물건명 가격 갯수 선언,
		 	5개 row로 각 cel마다 물건명/가격은 input, 갯수는 0~9까지 있는 select 만들기 
1) 기본화면 구성
...
4) script 처리가 필요한 부분 처리.		 	
	...
	<h2>사원정보</h2>
	<%
	// Emp(int empno, String ename, int sal, int deptno)
	List<Emp> elist = new ArrayList<Emp>();
	elist.add(new Emp(7781,"홍길동",3000,10));
	elist.add(new Emp(7782,"김길동",3000,20));
	elist.add(new Emp(7783,"신길동",3000,10));
	elist.add(new Emp(7784,"오길동",3000,40));
	%>
	<table>
		<col width="10%">
		<col width="20%">
