...
public class A01_Basic {
	...
	7. 논리
		...
		age>=14 && age<=18  두가지 비교연산자가 모두 true
			일때, true
			ex) id, pass동일시..
			id == "himan" && pass = "7777" //이렇게 하면 안됨(자바만)
			id.equals("himan") && pass.equals("7777") //이렇게 해야됨!
			
			*** 주의
			자바는 위와같이 비교연산자로 문자열 비교하면 안된다.
			"" ==> new Stringk("") ==> heap영역 사용.
	...
	1. 연산자의 방향은 기본적으로 
		비교연산자 : 왼쪽에서 오른쪽으로 처리된다.
			age>=14 & age<=18 :
				age가 14미만이더라도 age<=18를 check한다.
			age>=14 && age<=18
				age가 14미만이면 age<=18를 check하지 않는다.
	...
	2. 산술연산자 우선순위
		...
		2) * / /
			...
			2+5*7 ==> 37
		3) +/-
		cf) 산술연산자의 우선 순위는 결과의 차이가 있는 경우에
		의미가 있기에 선언하고, 곲셈과 나눗셈, 덧셈과 뺄셈은
		우선처리 순서에 따라 결과가 동일하기에 의미가 없다.
		ps) ()-소괄호
			{}-중괄호
			[]-대괄호
