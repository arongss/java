...
public class A03_ModelController {
	...
	RequestParam(value="kor", defaultValue = "0") 
		요청값이 없을 때, default값으로 0을 설정하고, 요청값이 있을 때는 해당 요청값을
	...
	public String student03(
		...
		d.addAttribute("avg", kor+eng+math);
		// a12_studentPoint.jsp
		return "WEB-INF\\views\\a01_start\\a12_studentPoint.jsp";
	}
	// ex) http://localhost:7080/springweb/buyProduct.do
	//		구매물건:[   ], 구매가격:[   ], 구매갯수:[   ]   [구매] 모델로 총비용 계산
	//		구매 클릭시 ==> 하단에 구매물건은 @@@ 총비용은 @@@ 라고 출력되게 하세요
	...
	<input name="price"/>
	<input name="cnt"/>
	<h2>구매물건은 ${param.pname} 총비용은 ${totPay}</h2>
	 */
}