...
numList03.remove(80)
print("80 삭제 후:", numList03) # 첫번째 데이터 삭제
# count(찾을 값) : 해당 찾을 값 갯수 확인
print("90 갯수 확인:",numList03.count(90))
# sort(), sort(reverse=True)
print("정렬:",numList03.sort())
numList03.sort(reverse=True)
print("역정렬:",numList03)
# reverse() : 정렬상관없이 순서를 뒤에서 부터 출력
...
#    역정렬을 시키고, 2번째 데이터 삭제 후 마지막 데이터를 삭제 후
#    데이터 출력
import random as r
diceList = []
for cnt in range(5):
    diceList.append(r.randrange(1,7))
print("5번 던진후:",diceList)
diceList.insert(2, r.randrange(1,7))
print("3번째 위치 추가 후:",diceList)
diceList.sort(reverse=True)
print("역정렬 후:",diceList)
