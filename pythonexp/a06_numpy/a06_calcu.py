...
@author: SAMSUNG
# ndarray의 산술연산자
1. 배열의 숫자 데이터 연산은 배열의 각 요소에 연산결과를 리턴한다.
2. 동일 크기를 갖는 배열은 동일한 위치의 요소 데이터끼리 연산 수행한다.
    list01 = np.array([1,2,3])
    list02 = np.array([3,4,5])
    list03 = list01*2 ==> 각 요소들 마다 데이터를 *2로 처리한다.
                [2,4,6]
    list04 = list01+list02 ==> 각 요소의 같은 index 위치에 데이터
        끼리 연산 처리 ==> [4,6,8]
# ndarray의 비교연산자
1. 배열의 각데이터를 비교하여 논리값을 저장하는 연산처리를 말한다.
2. ex) 
    boolList = np.equal(list01, list02)
        두 배열데이터의 각 요소들을 비교하여 bool값의 배열을 만들다
        1==3    2==4    3==5
        [False, False, False]
3. 종류
...
print("not_equal(!=):", np.not_equal(list01, list02))
...
#    3) 위 내용을 이름과 함께 평균 점수 출력 및 평균점수가 70점
#        이상으로 합격여부 출력
snames = np.array(["홍길동","김길동","신길동"])
kors = np.array([90,80,70])
engs = np.array([80,80,70])
maths = np.array([60,50,50])
