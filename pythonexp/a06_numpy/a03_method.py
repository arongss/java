...
#    마지막값이 포함시키지 않으려면 endpoint속성 활용
ar07 = np.linspace(1,10,4, endpoint=False)
print(ar07)
# ex1) 0~100 사이의 구간을 5개 구간을 나누어 출력
ar08 = np.linspace(0,100,5)
print(ar08)
# ex2) 1~7 사이의 구간을 6개로 나누어서 출력하되
#    마지막 값 포함하지 않게 처리
ar09 = np.linspace(1,7,6, endpoint=False)
print(ar09)
'''
# 행열의 변경되는 구조 만들기 reshape
1. 일차원의 특정 데이터를 행/열을 지정해서 2차원의 데이터로 변형시켜주는 역할을 한다.
2. 기본형식
    ndarray.reshape(행, 열)
'''
ar10 = np.arange(10)
...
1. 행/열데이터를 0으로 처리되는 가운데 대각선 데이터 처리를 하되
...
# ex1) 1~16까지 4X4 데이터에서 사선데이터를 추출하여 출력하세요..
ar19 = np.arange(1,17).reshape(4,4)
print(ar19)
print(np.diag(ar19))
# ex2) 100~76까지 5X5 데이터에서 사선데이터(두칸아래행)를 추출하여 출력하세요..
